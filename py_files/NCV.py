...
outer_cv = KFold(n_splits=7, shuffle=True, random_state=42)
...
# predicted_subgroups_svm = best_svm_model.predict(test_data)
# predicted_subgroups_knn = best_knn_model.predict(test_data)

# Compare predictions and count correct classifications
# correct_classifications = sum(predicted_subgroups_svm == predicted_subgroups_knn)
...
# predictions_df = pd.DataFrame({'Sample': test_samples, 'Subgroup': predicted_subgroups_svm})
