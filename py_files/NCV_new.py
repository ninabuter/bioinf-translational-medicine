...
for name, (classifier, param_grid) in classifiers.items():
    ...
    print("PRINTING THE ENTIRE MODEL")
    print(grid_search)
    print("========================")
    pickle.dump(grid_search, open(name + '.pkl', 'wb'))

# Save the best performing model (best_rf_model) as a pickle file
...
predicted_subgroups_rf = best_rf_model.predict(test_data)
predicted_subgroups_knn = best_knn_model.predict(test_data)

# Compare predictions and count correct classifications
correct_classifications = sum(predicted_subgroups_rf == predicted_subgroups_knn)

# Write the number of correct classifications to a text file
with open("estimate.txt", "w") as file:
    file.write(str(correct_classifications))

# Write the predictions of the best classifier (RF) to a text file
predictions_df = pd.DataFrame({'Sample': test_samples, 'Subgroup': predicted_subgroups_rf})
